...
export class StorageRepositoryImpl implements StorageRepository {
  ...
  async createProfilePreSignedUrl (userId: string): Promise<string> {
    ...
    if (existingFile) {
      ...
      return await this.getFileInFolder(existingFile)
    }
    console.log(`File does not exist: ${filename}`)
    return await this.createFileInFolder(profileImageFolderKey, filename)
  }
  ...
  async createPostPreSignedUrls (userId: string, postId: string, images: string[]): Promise<string[]> {
    ...
    const preSignedUrls = await Promise.all(
      images.map(async (filename) => {
        return await this.createFileInFolder(postFolderKey, filename)
      })
    )
    ...
    try {
      ...
      const preSignedUrls = await Promise.all(
        fileKeys.map(async (fileKey) => {
          return await this.getFileInFolder(fileKey)
        })
      )
  ...
  private async getFileInFolder (existingFile: string): Promise<string> {
    ...
    return await getSignedUrl(this.s3Client, command, { expiresIn: 240 }) // you have four minutes to change the image
  }

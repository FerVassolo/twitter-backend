...
export class SocketHandler {
  ...
  private readonly connectedUsers: Map<string, Set<string>> // Map<UserId, Set<SocketId>>
  ...
  private async handlePrivateMessage (socket: Socket, { to, message }: { to: string, message: string }): Promise<void> {
    ...
    await this.sendMessage(socket, senderUserId, to, message)
  }
  ...
  private getSenderUserId (socket: Socket): string | null {
    return (
      Array.from(this.connectedUsers.entries()).find(
    ...
    if (userSockets && userSockets.size > 0) {
      userSockets.forEach((socketId) => {
        if (socketId !== socket.id) {
          // Doesn't send to the socket sending the message
  ...
  private handleDisconnect (socket: Socket): void {
    const userId = Array.from(this.connectedUsers.entries()).find(([, sockets]) => sockets.has(socket.id))?.[0]

...
import { db, Logger, socketAuth } from "@utils";
...
export class SocketHandler {
  ...
  private async handlePrivateMessage(
    ...
    if (!areFriends) {
      ...
      return; // TODO: emit a message to the sender indicating him that he cannot send the message.
    }
  ...
  private async sendMessage(socket: Socket, senderUserId: string, to: string, message: string): Promise<void> {
    ...
    await this.saveOnDB(socket.id, message, senderUserId, to);
  ...
  private async saveOnDB(socketId: string, message: string, senderUserId: string, to: string) {

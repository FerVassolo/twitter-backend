...
describe('Post Service', () => {
  ...
  beforeEach(() => {
    ...
    } as const // Adding `as const` ensures immutability and prevents modification.
  ...
  it('should delete post', async () => {
    postRepositoryMock.getById.mockResolvedValue({ id: 'id', authorId: '1', content: 'content', images: [], createdAt: new Date() })
    postRepositoryMock.delete.mockResolvedValue(undefined)

    await postService.deletePost('1', 'id')

    expect(postRepositoryMock.delete).toHaveBeenCalledWith('id')
  })

  it('should throw forbidden error when trying to delete post the user didn\'t create', async () => {
    postRepositoryMock.getById.mockResolvedValue({ id: 'id', authorId: '1', content: 'content', images: [], createdAt: new Date() })
    postRepositoryMock.delete.mockResolvedValue(undefined)

    await expect(postService.deletePost('2', 'id'))
      .rejects.toThrow('Forbidden. You are not allowed to perform this action')

    expect(postRepositoryMock.delete).not.toHaveBeenCalled()
  })
